...
if __name__ == '__main__':
    ...
    # 初始化数据集
    dataSetList,data0,data1 = loadData(mu0, sigmod0, mu1, sigmod1, alpha0, alpha1)

    #打印设置的参数
    print('---------------------------')
    print('the Parameters set is:')
    print('alpha0:%.5f, mu0:%.5f, sigmod0:%.5f, alpha1:%.5f, mu1:%.5f, sigmod1:%.5f' % (
        alpha0, mu0,sigmod0,alpha1, mu1,sigmod1
    ))

    # 开始EM算法，进行参数估计
    p_alpha0, p_mu0, p_sigmod0, p_alpha1, p_mu1, p_sigmod1 = EM_Train(dataSetList,iter=600)

    # 打印参数预测结果
    print('----------------------------')
    print('the Parameters predict is:')
    print('alpha0:%.5f, mu0:%.5f, sigmod0:%.5f, alpha1:%.5f, mu1:%.5f, sigmod1:%.5f' % (
        p_alpha0, p_mu0, p_sigmod0, p_alpha1, p_mu1, p_sigmod1
    ))
    ...
    plt.scatter(data0,[1]*(length//2),marker='x')
    plt.scatter(data1, [1.2] * (length // 2), marker='x')
    #xs = list(range(1.4,1.95,0.05))
    def normfun(x, mu, sigma):
        pdf = np.exp(-((x - mu) ** 2) / (2 * sigma ** 2)) / (sigma * np.sqrt(2 * np.pi))
        return pdf

    # x的范围为60-150，以1为单位,需x根据范围调试
    u1 = np.arange(0.5, 2.0, 0.02)
    # x数对应的概率密度
    y1 = normfun(u1, mu0,sigmod0)
    y2 = normfun(u1, mu1, sigmod1)
    y3 = normfun(u1, p_mu0, p_sigmod0)
    y4 = normfun(u1, p_mu1, p_sigmod1)
    # 参数,颜色，线宽
    plt.plot(u1, y1, color='g', linewidth=3)
    plt.plot(u1, y2, color='r', linewidth=3)
    plt.plot(u1, y3, linewidth=2)
    plt.plot(u1, y4, linewidth=2)
    plt.show()